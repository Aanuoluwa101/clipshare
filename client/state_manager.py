...
def state_manager(state):
    ...
    while True: 
        ...
        if state.gateway and not state.client.is_connected and not state.shutdown_signal.is_set() and state.connect_signal.is_set():    
        ...
        elif state.client.is_connected and (not state.gateway or state.shutdown_signal.is_set()): 
