...
class UI:
    ...
    def update(self):
        try:
            ...
            if clipboard != self.state.server.clipboard.local and clipboard !=self.state.server.clipboard.content:
            ...
            #clipboard state
            self.body.clipboard_state.config(text=self.state.server.clipboard.send_state)
    ...
    def show_server_details(self):
        ...
        if self.state.server.is_online and not self.showing_server_details:
            ...
            name, address, passcode = self.state.server.name, self.state.server.address, self.state.server.passcode

            server_details = ServerDetails(self.root, name, address, passcode, on_server_details_close)
        elif not self.state.server.is_online:
            self.state.error_message = "Offline! Connect to a wifi"
